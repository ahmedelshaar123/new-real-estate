<?php
/**
 * Created by PhpStorm.
 * User: bg
 * Date: 7/6/2020
 * Time: 1:22 PM
 */
return [
    'home' => 'الرئيسية',
    'news' => 'الأخبار',
    'our_latest_news' => 'أخر الأخبار',
    'follow_our_latest_news_&_latest_updates' => 'تابع أخر الأخبار والتحديثات',
    'no_services' => 'خدماتنا',
    'no_news' => 'لا توجد أخبار',
    'no_partners' => 'لا يوجد شركاء',
    'our_partners' => 'شركاؤنا',
    'our_services' => 'خدماتنا',
    'company_team' => 'فريق العمل',
    'team' => 'الفريق',
    'we_are_an_integrated_team_and_we_strive_to_provide_the_best' => 'نحن فريق متكامل و نسعى الى تقديم اﻷفضل',
    'no_employees' => 'لا يوجد موظفين',
    'real_estate' => 'الشركة العقارية',
    'fast_links' => 'روابط سريعة',
    'real_estates_news' => 'أخبار الشركات العقارية',
    'contact_us' => 'اتصل معانا',
    'address' => 'العنوان',
    'phone' => 'الجوال',
    'do_not_miss_to_follow_up_and_subscribe_to_our_new_services_or_you_can_follow_us_now' => 'لا تنسي المتابعة والاشتراك لأخر خدماتنا الجديدة أو يمكنك متابعتنا',
    'all_rights_reserved' => 'كل الحقوق محفوظة',
    'email' => 'البريد الالكتروني',
    'about_us' => 'عن الموقع',
    'a_real_estate_company_that_seeks_to_provide_real_estate_services_with_credibility_to_achieve_the_highest_levels_of_customer_acceptance' => 'شركة عقارية تسعى فى تقديم الخدمات العقارية بمصداقية بتحقيق أقصي درجات القبول لدى العملاء',
    'vision' => 'الرؤية',
    'message' => 'الرسالة',
    'name' => 'الاسم',
    'title' => 'العنوان',
    'send' => 'ارسال',
    'from_services_that_company_provides' => 'من الخدمات التي تقدمها الشركة',
    'no_featured_real_estates' => 'لا توجد عقارات مميزة',
    'experience_years' => 'سنوات الخبرة',
    'rewards_' => 'الجوائز',
    'successful_deals' => 'الصفقات الناجحة',
    'no_bottom_sliders' => 'لا توجد شرائح بالأسفل',
    'show_more' => 'استعراض المزيد',
    'start' => 'البدء',
    'our_news' => 'أخبارنا',
    'no_top_sliders' => 'لا توجد شرائح بالأعلي',
    'work_times' => 'مواعيد العمل',
    'rewards_&_trophies' => 'الجوائز والأوسمة',















];
